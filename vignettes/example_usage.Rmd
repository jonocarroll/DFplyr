...
`r Biocpkg("DFplyr")` is a `R` package available via the [Bioconductor](http://bioconductor.org) repository for packages and can be downloaded via `BiocManager::install()`:
...
`r Biocpkg("DFplyr")` is inspired by `r CRANpkg("dplyr")` which implements a wide 
variety of common data manipulations (`mutate`, `select`, `filter`) but which 
only operates on objects of class `data.frame` or `tibble` (from `r CRANpkg("tibble")`).

When working with `r Biocpkg("S4Vectors")` `DataFrame`s - which are frequently used 
as components of, for example `r Biocpkg("SummarizedExperiment")` objects - a common 
workaround is to convert the `DataFrame` to a `tibble` in order to then use 
`r CRANpkg("dplyr")` functions to manipulate the contents, before converting back 
to a `DataFrame`. 
...
numeric columns using a quosure style lambda function, in this case squaring them
...
Other verbs are similarly implemented, and preserve row names where possible. For 
example, selecting a liimted set of columns using non-standard evaluation (NSE)
...
These also work for grouped objects, and also preserve the rownames, e.g. selecting 
the first two rows from _each group_ of `gear`
...
Behaviours are ideally the same as those of `r CRANpkg("dplyr")` wherever possible, 
for example a grouped tally
...
We hope that `r Biocpkg("DFplyr")` will be useful for your research. Please use the following information to cite the package and the overall approach. Thank you!
